...
#define DEF_ENGINE_INIT_KEY 1784255353
#define DEF_ENGINE_INIT_FLAG 23175
...
bool CDTEngineV8::Init(const DT_STREAM_DESC* sd_init, const DT_STREAM_DESC* sd_fontmap, DT_INIT_INFO* init_info)
{
	...
	if (txMakeCachedFontmap(Engine, sd_fontmap, &DefaultFontmap, 0) != 1) { Exit(); return false; }
...
void CDTEngineV8::Exit()
{
	...
	if (Engine != DV_NULL) pdEngineExt(Engine);
	// release memory that txMakeCachedFontmap allocated for the cached font map
...
bool CDTDocV8::SetFontmap(const DT_STREAM_DESC* sd_fontmap)
{
	...
	if (txMakeCachedFontmap(Engine->GetPDEngine(), sd_fontmap, &Fontmap, 0) != 1) return false;
	return true;
...
#endif
	if (cleanup_flags & 4) pdxDocDiscardDescriptions(PDDoc, "!"); /* no longer critical since D-Type 7.0.1.2 uses pdObjFindByNameAndDescPlus */
...
DT_SLONG CDTDocV8::TransferWithCleanup(DT_PDDOC pd_doc, DT_UWORD cleanup_flags /* = 0 */, bool erase /* = false */)
{
	...
	if (cleanup_flags & 4) pdxDocDiscardDescriptions(pd_doc, "!"); /* no longer critical since D-Type 7.0.1.2 uses pdObjFindByNameAndDescPlus */
