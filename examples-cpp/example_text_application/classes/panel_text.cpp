...
private:
	...
	void KeyDown(DT_SLONG key, DT_ULONG flags, DT_ID_ULONG char_code)
	{
		...
		else if (key == 127 /*DELETE*/) Event_TextPaste(TX_PASTE_DELETE_REFORMAT, TX_AUTO, DV_NULL, 0);
		else if (char_code > 0)
		{
			DT_ID_ULONG unicode_code = GetFilteredChar(char_code, 1); if (unicode_code == 0) return;
			DT_UBYTE unicode_buffer[TX_BUFFER_FORMAT_4]; DF_WRITE_ULONG_LE(unicode_buffer, unicode_code);
			Event_TextPaste(TX_PASTE_TEXT_REFORMAT, TX_UCS4_LE, unicode_buffer, TX_BUFFER_FORMAT_4);
		}
	}
