...
public:
	...
	void Load(DT_PDENGINE pd_engine, const DT_CHAR* filename)
	{
		CDTPanelStatic::Load(pd_engine, filename);
...
public: // Events
	...
	void Event_CharDown(DT_ID_ULONG char_code, DT_ULONG flags)
	{
		KeyDown(0, flags, char_code);
	}
...
private:
	...
	void KeyDown(DT_SLONG key, DT_ULONG flags, DT_ID_ULONG char_code)
	{
		...
		{
			DT_ID_ULONG unicode_code = GetFilteredChar(char_code, 0); if (unicode_code == 0) return;
			DT_UBYTE unicode_buffer[TX_BUFFER_FORMAT_4]; DF_WRITE_ULONG_LE(unicode_buffer, unicode_code);
			FormEngine->EvnF_KeyChar(TX_PASTE_TEXT, unicode_buffer, 1, TX_BUFFER_FORMAT_4);
		}
	}
